...
#else
static void $(...) {}
#define trace(x) if(false) {x;} else {}
#endif
...
namespace {
	...
	public:
		...
		void Relinquish() { relinquished = true; }
		~SocketCloser() {
			if (!relinquished) {
...
#pragma pack(pop)
	...
	void OpenSocket(ServerAddress &server_address) {
		...
		if (server_address.family != AF_INET && server_address.family != AF_INET6 ||
			server_address.port < 1000 ||
			server_address.version != version) {
	...
	template <typename chan_t, typename text_t, typename file_t> void Writer(chan_t const *channel, text_t const *text, file_t const *file, int line, int severity, FILETIME const &timestamp) {
	...
	template <typename chan_t, typename file_t> void Log_OutputInternal(chan_t const *channel, WCHAR const *text, file_t const *file, int line, int severity, va_list v) {
		if (IsDebuggerPresent() && !no_plugin) {
			FILETIME timestamp;
...
namespace chs {
	namespace Log {
		namespace Internal {
			...
			void Output(WCHAR const *channel, WCHAR const *text, WCHAR const *file, int line, int severity, ...) { va_list v; va_start(v, severity); Log_OutputInternal(channel, text, file, line, severity, v); }
			void Output(WCHAR const *channel, WCHAR const *text, CHAR const *file, int line, int severity, ...) { va_list v; va_start(v, severity); Log_OutputInternal(channel, text, file, line, severity, v); }
			void Output(WCHAR const *channel, CHAR const *text, WCHAR const *file, int line, int severity, ...) { va_list v; va_start(v, severity); Log_OutputInternal(channel, text, file, line, severity, v); }
			void Output(WCHAR const *channel, CHAR const *text, CHAR const *file, int line, int severity, ...) { va_list v; va_start(v, severity); Log_OutputInternal(channel, text, file, line, severity, v); }
			void Output(CHAR const *channel, WCHAR const *text, WCHAR const *file, int line, int severity, ...) { va_list v; va_start(v, severity); Log_OutputInternal(channel, text, file, line, severity, v); }
			void Output(CHAR const *channel, WCHAR const *text, CHAR const *file, int line, int severity, ...) { va_list v; va_start(v, severity); Log_OutputInternal(channel, text, file, line, severity, v); }
			void Output(CHAR const *channel, CHAR const *text, WCHAR const *file, int line, int severity, ...) { va_list v; va_start(v, severity); Log_OutputInternal(channel, text, file, line, severity, v); }
			void Output(CHAR const *channel, CHAR const *text, CHAR const *file, int line, int severity, ...) { va_list v; va_start(v, severity); Log_OutputInternal(channel, text, file, line, severity, v); }
		}
	}
}
